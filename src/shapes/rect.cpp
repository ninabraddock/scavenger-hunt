...
bool Rect::isOverlapping(const Rect &r1, const Rect &r2) {
    ...
    return r1.getRight() > r2.getLeft() &&
           r1.getLeft() < r2.getRight() &&
           r1.getTop() > r2.getBottom() &&
           r1.getBottom() < r2.getTop();

    //given a pos x,y cartesian plane where both rect are in the first quadrant
    //1. if r1 is to the left of r2 or switch the box location with the other
    if(r2.getLeft() > r1.getLeft() || r1.getLeft() > r2.getLeft())
        return true;
    ///1. if r1 is on top of r2 or switch the box location with the other
    if(r1.getTop() > r2.getTop() || r2.getTop() > r1.getTop())
        return true;
